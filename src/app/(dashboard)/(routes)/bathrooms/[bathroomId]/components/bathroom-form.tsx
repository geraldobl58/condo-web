...
type BathroomFormValues = z.infer<typeof formShema>;

export const BathroomForm = ({ initialData }: BathroomFormProps) => {
  ...
  const onSubmit = async (data: BathroomFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/api/bathrooms/${params.bathroomId}`, data);
      } else {
        await axios.post(`/api/bathrooms`, data);
      }
      router.refresh();
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/bathrooms/${params.bathroomId}`);
      router.refresh();
      router.push("/");
