...
type BedroomFormValues = z.infer<typeof formShema>;

export const BedroomForm = ({ initialData }: BedroomFormProps) => {
  ...
  const form = useForm<BedroomFormValues>({
  ...
  const onSubmit = async (data: BedroomFormValues) => {
    try {
      setLoading(true);
      if (initialData) {
        await axios.patch(`/api/bedrooms/${params.bedroomId}`, data);
      } else {
        await axios.post(`/api/bedrooms`, data);
      }
      router.refresh();
  ...
  const onDelete = async () => {
    try {
      setLoading(true);
      await axios.delete(`/api/bedrooms/${params.bedroomId}`);
      router.refresh();
      router.push("/");
